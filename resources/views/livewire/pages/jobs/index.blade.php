                            ...
                            @foreach ($jobs as $job)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        @if($job['logo'] != '')
                                            <img src="{{ asset('storage/' . $job['logo']) }}" class="h-12 w-auto block mx-auto" alt="Job Logo">
                                        @else
                                            <img src="{{ asset('storage/company_logos/default.svg') }}" class="h-12 w-auto block mx-auto" alt="Job Logo">
                                        @endif
                                    </td>
                                    <td><span class="px-4 py-3 font-medium text-gray-900">{{ $job['company_name'] }}</span></td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center flex-wrap gap-2">
                                            @foreach ($job['job_skills'] as $skill)
                                                <span class="inline-block bg-gray-200 rounded-full px-2 py-0.5 text-xs font-medium text-gray-700">{{ $skill['skill']['name'] }}</span>
                                            @endforeach
                                        </div>
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <a href="{{ route('admin.jobs.edit', ['jobId' => $job['id'] ]) }}"
                                            class="text-sm px-3 py-1.5 rounded hover:bg-slate-100 transition-colors text-blue-700">
                                             Edit
                                         </a>
                                        <button wire:click="deleteJob({{$job['id']}})" class="text-sm px-3 py-1.5 rounded hover:bg-slate-100 transition-colors text-red-600">Delete</button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
